...
async function getOrgByAuth0Id(c: Context<Env>, auth0OrgId: string): Promise<{ id: number; auth0_org_id: string; org_type: string; name: string } | null> {
  const row = await c.env.DB.prepare(
    `SELECT id, auth0_org_id, org_type, name FROM Organizations WHERE auth0_org_id = ?`
  )
    .bind(auth0OrgId)
    .first<any>();
  return row ?? null;
...
async function getOrgIdByAuth0Id(c: Context<Env>, auth0OrgId?: string | null): Promise<number | null> {
  ...
  const row = await c.env.DB.prepare(
    `SELECT id FROM Organizations WHERE auth0_org_id = ?`
  )
    .bind(auth0OrgId)
    .first<{ id: number }>();
  return row?.id ?? null;
...
app.patch('/organizations/:orgId', async (c) => {
  ...
  try {
    const exists = await c.env.DB.prepare(
      'SELECT auth0_org_id FROM Organizations WHERE auth0_org_id = ?'
    )
      .bind(orgId)
      .first();
...
app.get('/schedules', async (c) => {
  ...
  try {
    ...
    const list: components['schemas']['PickupSchedule'][] = (rs.results || []).map((r: any) => ({
      ...
      default_estimated_weight_kg: r.default_estimated_weight_kg != null ? Number(r.default_estimated_weight_kg) : undefined,
    }));
    return c.json(list);
  } catch {
    return c.json({ error: 'Server error' }, 500);
...
app.post('/schedules', async (c) => {
  ...
  try {
    ...
    const dto: components['schemas']['PickupSchedule'] = {
      ...
      default_estimated_weight_kg: row.default_estimated_weight_kg != null ? Number(row.default_estimated_weight_kg) : undefined,
    };
...
app.patch('/schedules/:scheduleId', async (c) => {
  ...
  try {
    const exists = await c.env.DB.prepare(
      `SELECT id, supplier_id FROM PickupSchedules WHERE id = ?`
    )
      .bind(scheduleId)
      .first<any>();
    if (!exists) return c.json({ error: 'Not Found' }, 404);

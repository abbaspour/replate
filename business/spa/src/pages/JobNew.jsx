...
export default function JobNew() {
  ...
  if (!canCreate) {
    return <div className="container"><p>You do not have permission to create jobs.</p></div>
  }
  ...
  async function onSubmit(e) {
    ...
    try {
      const body = {
        ...
        food_category: form.food_category.split(',').map(s => s.trim()).filter(Boolean),
      ...
      <div className="card">
        ...
        <form onSubmit={onSubmit}>
          <div className="row">
            ...
            <input type="datetime-local" required value={form.pickup_window_start} onChange={e => upd('pickup_window_start', e.target.value)} />
          </div>
          <div className="row">
            <label>Pickup window end</label>
            <input type="datetime-local" required value={form.pickup_window_end} onChange={e => upd('pickup_window_end', e.target.value)} />
          </div>
          <div className="row">
            <label>Food categories (comma separated)</label>
            <input type="text" placeholder="bakery, produce" value={form.food_category} onChange={e => upd('food_category', e.target.value)} />
          </div>
          <div className="row">
            <label>Estimated weight (kg)</label>
            <input type="number" min="1" step="0.5" value={form.estimated_weight_kg} onChange={e => upd('estimated_weight_kg', e.target.value)} />
          </div>
          <div className="row">
          ...
          <div className="row">
            ...
            <input type="text" value={form.community_org_id} onChange={e => upd('community_org_id', e.target.value)} />
          </div>
          <div>

...
export default function Organization() {
  ...
  if (!canRead) return <div className="container"><p>You do not have permission to view organization.</p></div>
          ...
          <form onSubmit={onSave}>
            ...
            {org.org_type === 'supplier' && (
              <div className="row">
                ...
                <input value={org.pickup_address || ''} onChange={e => setOrg(o => ({...o, pickup_address: e.target.value}))} />
              </div>
            )}
            {org.org_type === 'community' && (
              <div className="row">
                <label>Delivery address</label>
                <input value={org.delivery_address || ''} onChange={e => setOrg(o => ({...o, delivery_address: e.target.value}))} />
              </div>
            )}
            {org.org_type === 'logistics' && (
              <>
                <div className="row">
                  ...
                  <input value={org.coverage_regions || ''} onChange={e => setOrg(o => ({...o, coverage_regions: e.target.value}))} />
                </div>
                <div className="row">
                  <label>Vehicle types (comma separated)</label>
                  <input value={(org.vehicle_types||[]).join(', ')} onChange={e => setOrg(o => ({...o, vehicle_types: e.target.value.split(',').map(s=>s.trim()).filter(Boolean)}))} />
                </div>
              </>
            )}

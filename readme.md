...
APIs are built with Hono, accept Auth0-issued access tokens, and deploy as Cloudflare Workers.
...
Note: The diagram below illustrates the logical data model; in implementation this is split across the three D1
...
checking invitation status, and listing organizations). Admin API runs as a Cloudflare Worker using Hono and uses its
Admin D1 database (admin/db) and the Auth0 Management API.
...
- Runtime configuration is externalized via `public/auth_config.json` and per-project `.env` files (often populated by
  Terraform).
- APIs are Hono workers per domain with restricted CORS and bearer verification.
...
- Admin Application (SPA/Console):
    ...
    - audience: `admin.api`
    - scope: openid profile email
    - permissions: read:organizations update:organizations create:organizations read:sso_invitations create:
...
Audiences, scopes, and per-app permissions are defined in `## Common Auth0 configuration` above.

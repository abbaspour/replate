 ...
 * - clientId: The client ID for companion m2m client with read & update users scope
 * - clientSecret: The client secret for companion m2m client with read & update users scope
 *
 * NPM Dependencies:
 *  - auth0
 */

const CACKE_KEY = 'silent-linking-management-token';
...
exports.onExecutePostLogin = async (event: Event, api: PostLoginAPI) => {
    ...
    let {value: token} = api.cache.get(CACKE_KEY) || {};
    ...
    if (!token) {
        ...
        try {
            ...
            const result = api.cache.set(CACKE_KEY, token, {ttl: data.expires_in * 1000});
